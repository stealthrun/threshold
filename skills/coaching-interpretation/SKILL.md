...
1. **Deterministic signals first.** `stimulus_quality`, `vs_plan`, and `key_signal` are
   computed in plain Python (`scripts/`), not by the model. Verifiable and free.
...
- [ ] `scripts/derive_signals.py` — the deterministic `stimulus_quality` / `vs_plan` /
      `key_signal` derivation, distilled from the private lab.
